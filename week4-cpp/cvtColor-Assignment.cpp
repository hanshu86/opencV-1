...
void rgb_to_hsv(float r, float g, float b, uchar *hue, uchar *sat, uchar *value) 
{
    ...
    if (cmax == cmin)
      ...
      h = ((120 + (60* (b - r)))/(cmax - cmin)) ;
    else if (cmax == b)
        //240+60(R−G)/(V−min(R,G,B))
      h = ((240 + (60* (r - g)))/(cmax - cmin)) ;
    // if cmax equal zero
      if (cmax == 0)
         s = 0;
      else
    ...
    for (int i = 0; i < image.cols; i++) {
        for (int j = 0; j < image.rows; j++) {
            ...
            for(int k = 0; k < image.channels(); k++) {
                // calculate pixValue
                if(k == 0)intensityChange.val[k] = hue/2;
                if(k == 1)intensityChange.val[k] = sat;
                if(k == 2)intensityChange.val[k] = value;
            }
        }
    }
